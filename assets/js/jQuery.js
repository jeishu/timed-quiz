// DOM Elements
    ...
    let formSumbitBtnEl = document.getElementsByClassName(".formSumbitBtn");
...
function answerCheck(e) {
    ...
    if (userChoice == "choiceAbtn" || userChoice == "choiceBbtn" || userChoice == "choiceCbtn" || userChoice == "choiceDbtn") {
        ...
        if (answerChoice) {
            ...
            kaomoji.textContent = "	ヽ(*⌒▽⌒*)ﾉ";
            quizPageEl.append(kaomoji); // how to remove?
        }
        else {
            kaomoji = document.createElement("p");
            kaomoji.textContent = "	ヽ(　￣д￣)ノ";
            quizPageEl.append(kaomoji); // how to remove?
            displayTime.textContent = displayTime.textContent - 10; // Issue, not working..?
        }
        questionChange();
    }
}
...
formSumbitBtnEl.submit(function(event) {
